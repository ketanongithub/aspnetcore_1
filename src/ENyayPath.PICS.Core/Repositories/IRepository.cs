...
namespace ENyayPath.PICS.Core.Repositories
{
    ...
    public interface IRepository<TEntity, TPrimaryKey> : IRepository, ITransientDependency where TEntity : class, IEntity<TPrimaryKey>
    {
        ...
        //
        // Summary:
        //     Used to run a query over entire entities. App.Domain.Uow.UnitOfWorkAttribute
        //     attribute is not always necessary (as opposite to App.Domain.Repositories.IRepository`2.GetAll)
    ...
    //    // 🔹 IQueryable for advanced queries
    //    IQueryable<TEntity> GetAll();
    ...
    //    void SetTenantContext(int tenantId);
    //    int? CurrentTenantId { get; }

    //    // 🔹 Audit logging hooks
    //    event Func<TEntity, Task> OnEntityInserted;
    //    event Func<TEntity, Task> OnEntityUpdated;
    //    event Func<TEntity, Task> OnEntityDeleted;
    //}
}
